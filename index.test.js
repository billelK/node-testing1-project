...
describe('[Exercise 1] trimProperties', () => {
  ...
  test('[2] returns a copy, leaving the original object intact', () => {
    const input = {name: "  billel  ", profession: "    jewelcrafter"}
    const expected = {name: "billel", profession: "jewelcrafter"}
    const actual = utils.trimProperties(input)
    expect(actual).toEqual(expected)
...
describe('[Exercise 6] Car', () => {
  ...
  test('[17] refueling allows to keep driving', () => {
    ...
    expect(focus.drive(300)).toEqual(1000)
    expect(focus.drive(300)).toEqual(1200)
...
describe('[Exercise 7] isEvenNumberAsync', () => {
  test('[19] resolves true if passed an even number', async() => {
    expect(await utils.isEvenNumberAsync(4)).toEqual(true)
  })
